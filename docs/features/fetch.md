...
1. Заголовок: Fetch (с отображением даты отсечки, за которую происходит выборка, активных фильтров по source и topics, если есть; повторять формат вывода фильтров в заголовке Info Radar)
...
4. Footer с подсказкой по shortcuts
...
Когда пользователь вызывает команду fetch, отображается соответствующий экран, и запускается ожидание явного указания на старт выборки от пользователя (по шорткату s). 

Fetch проходит в 2 этапа:
1. Общее чтение topic'ов для получения ссылок на статьи. Progress bar отображает общее количество топиков к процессингу для всех источников в зависимости от установленных фильтров (отображается текущий анализируемый топик и общее количество топиков к обработке: Заголовок "Stage 1. Getting Articles")
2. Чтение конкретных статей по списку, выделенному на Stage 1. Progress bar отображает общее количество статей к enrichment для всех topic'ов, обработанных на Stage 1 (отображается текущая читаемая статья и общее количество статей к чтению: Заголовок "Stage 2. Reading Articles")

Можно отменить выполнение команды, нажав кнопку Esc. Выборка останавливается, но экран не закрывается, и пользователю в логах отображаются промежуточные результаты (с упоминанием о том, что fetch прерван пользователем). Повторный Esc прерывает выполнение команды и закрывает экран.
